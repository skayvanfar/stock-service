...
public class StockControllerIntegrationTest {
  ...
  @Sql(
      statements =
          "INSERT INTO TBL_STOCK (ID, NAME, CURRENT_PRICE, LAST_UPDATED) VALUES (10, 'LCC', 15.64, '2018-12-22T14:42:09.951687Z')",
      executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
  @Sql(statements = "DELETE FROM TBL_STOCK", executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
  ...
  public void getStockReturn404() {
    ResponseEntity<String> err =
        restTemplate.getForEntity(baseUrl.concat("/{id}"), String.class, 100);

    assertAll(
        () -> assertNotNull(err), () -> assertEquals(HttpStatus.NOT_FOUND, err.getStatusCode()) // ,
  ...
  public void createStockAndReturn201HttpStatus() {
    Stock stock =
        new Stock(
            10L, "CLC", BigDecimal.valueOf(15.64), Instant.parse("2018-12-22T14:42:09.951687Z"));
    ...
    Stock newStock =
        restTemplate.getForObject(baseUrl.concat("/{id}"), Stock.class, savedStock.getId());
    assertAll(
  ...
  @Sql(
      statements =
          "INSERT INTO TBL_STOCK (ID, NAME, CURRENT_PRICE, LAST_UPDATED) VALUES (10, 'CCC', 15.64, '2018-12-22T14:42:09.951687Z')",
      executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
  @Sql(statements = "DELETE FROM TBL_STOCK", executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
  public void updateStockAndReturn200HttpStatus() {
    ...
    HttpHeaders headers = new HttpHeaders();
    headers.setContentType(MediaType.APPLICATION_JSON);
    HttpEntity<BigDecimal> postRequest =
        new HttpEntity<>(priceUpdateDTO.getCurrentPrice(), headers);
    Stock updatedStock =
        restTemplate.patchForObject(baseUrl.concat("/{id}"), postRequest, Stock.class, 10);

    assertNotNull(updatedStock);

    Stock newStock =
        restTemplate.getForObject(baseUrl.concat("/{id}"), Stock.class, updatedStock.getId());
    assertAll(
        () -> assertNotNull(newStock),
  ...
  @Sql(
      statements =
          "INSERT INTO TBL_STOCK (ID, NAME, CURRENT_PRICE, LAST_UPDATED) VALUES (10, 'CHC', 15.64, '2018-12-22T14:42:09.951687Z')",
      executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
  @Sql(statements = "DELETE FROM TBL_STOCK", executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
